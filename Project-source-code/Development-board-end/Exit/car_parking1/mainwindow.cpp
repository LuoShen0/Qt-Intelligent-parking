    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->label->setText("感谢光临，请刷卡离开！");    //提示
    ui->label_11->setText("祝您一路平安！");       //提示

    ui->label->setScaledContents(true);
           movie = new QMovie("/play/chu.gif"); //新建一个播放器，传入动图的路径
...
void MainWindow::read_car()  //不停的读卡，当用户刷卡时会暂停5秒给用户拿走卡
{
    ...
    if(r_buf[2] == 0x00)
    {
        ...
        if(r_buf1[2] == 0)
        {
            ...
            QString msg = in_out_time + "/" + QString::number(car_id)+"/"+"out"; //拼接时间和卡号
            clien->write(msg.toUtf8());          //把时间、卡号以及出入标记发送给服务器
...
void MainWindow::readdata()  //接收服务器发过来的消费情况
{
    //读取所有数据
    QString xiaofei  = clien->readAll();

    if(xiaofei != nullptr)
    {

        //显示消费情况
         ui->label_7->setText(xiaofei);
    }
